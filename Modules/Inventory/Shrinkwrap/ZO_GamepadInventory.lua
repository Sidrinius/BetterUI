...
----------------------------------------------------------
-- the original code has been cloned from ZOS_GamepadInventory.lua, at API version 100023
...
function ZOS_GamepadInventory:OnActionsDialogFinished()
...
function ZOS_GamepadInventory:AddFilteredBackpackCategoryIfPopulated(filterType, iconFile)
...
function ZOS_GamepadInventory:RefreshCategoryList()
    ...
    for i, equipSlot in ZO_Character_EnumerateOrderedEquipSlots() do
        ...
        if not locked and not isListEmpty then
            ...
            data.filterType = weaponCategoryType ~= nil and ITEMFILTERTYPE_WEAPONS or ITEMFILTERTYPE_ARMOR or ITEMFILTERTYPE_JEWELRY
...
function ZOS_GamepadInventory_QuestItemSortComparator(left, right)
...
local function GetBestItemCategoryDescription(itemData)
    ...
    return ZOS_InventoryUtils_Gamepad_GetBestItemCategoryDescription(itemData)
end
...
function ZOS_GamepadInventory_OnInitialize(control)
    GAMEPAD_INVENTORY = ZOS_GamepadInventory:New(control)
end
